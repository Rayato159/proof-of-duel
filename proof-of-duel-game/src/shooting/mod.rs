...
pub struct ShootingEvent {
    pub player: usize,
    pub states: [ShootingStatesMesasge; 5],
}
    ...
    if let Some(data) = shooting_states.data.get_mut(current_key_index) {
        if keyboard_input.any_just_pressed(ALL_KEYS) {
            ...
            if shooting_states.is_last_key() {
                let shooting_states_message = shooting_states
                    .data
                    .iter()
                    .map(|s| ShootingStatesMesasge {
                        key: match s.key {
                            KeyCode::KeyQ => "Q",
                            KeyCode::KeyW => "W",
                            KeyCode::KeyE => "E",
                ...
                let _ = client
                    .connection_mut()
                    .send_message(&ClientMessage::ShootingCommand {
                        player_number: player_slecrion.0,
                        states: shooting_states_message.try_into().unwrap(),
                    });
